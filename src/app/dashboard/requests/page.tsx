...
import {
  Truck, User, MapPin, Clock, DollarSign, Building2, Inbox, Send,
  CheckCircle, XCircle, MessageSquare, AlertCircle, ArrowRight, Loader2
} from "lucide-react";
...
function MatchCard({ match, type, onRespond, onViewCounter, getTimeLeft, formatDate, isExpired }: any) {
      ...
      <Card className="border-primary/50">
          ...
          <div className="flex justify-between items-start">
            <div>
              <CardTitle className="text-lg flex items-center gap-2">
                ...
                {match.loadSnapshot.origin} &rarr; {match.loadSnapshot.destination}
              </CardTitle>
              <CardDescription>
                {match.loadSnapshot.cargo} &bull; {match.loadSnapshot.weight.toLocaleString()} lbs
              </CardDescription>
            </div>
            <Badge variant="outline" className="text-orange-600 border-orange-600">
              <Clock className="h-3 w-3 mr-1" />{getTimeLeft(match.expiresAt)}
            </Badge>
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-muted-foreground">{isForMyDriver ? "Your Driver:" : "Their Driver:"}</span>
            <span className="font-medium">{match.driverSnapshot.name}</span>
          </div>
          <div className="flex items-center gap-2">
            <DollarSign className="h-4 w-4 text-green-600" />
            <span className="text-lg font-bold text-green-600">${match.originalTerms.rate.toLocaleString()}</span>
          </div>
        ...
        <CardFooter>
          <Button className="w-full" onClick={() => onRespond(match)}>Respond to Request</Button>
        </CardFooter>
      </Card>
    );
  }

  if (type === 'sent-counter') {
    return (
          ...
          <div className="p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="font-medium line-through text-muted-foreground">${match.originalTerms.rate.toLocaleString()}</p>
              </div>
              <ArrowRight className="h-4 w-4 text-blue-600" />
              <div>
                <p className="text-xs text-blue-600 font-medium">Counter Rate</p>
                <p className="font-bold text-blue-600 text-lg">${match.counterTerms?.rate.toLocaleString()}</p>
              </div>
            </div>
            {match.counterTerms?.notes && (
              <p className="text-xs text-muted-foreground mt-2 pt-2 border-t">&quot;{match.counterTerms.notes}&quot;</p>
            )}
          </div>
          <div className="flex items-center gap-2 text-sm text-orange-600">
            <Clock className="h-4 w-4" /><span>{getTimeLeft(match.expiresAt)}</span>
          </div>
        </CardContent>
        <CardFooter>
          <Button className="w-full" onClick={() => onViewCounter(match)}>Review Counter Offer</Button>
        </CardFooter>
      </Card>
    );
  }
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <DollarSign className="h-4 w-4 text-green-600" />
            <span className="font-semibold text-green-600">${match.originalTerms.rate.toLocaleString()}</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  // Past card (incoming or sent)
  const expired = isExpired(match);
  const status = expired && match.status === 'pending' ? 'expired' : (expired ? 'expired' : match.status);
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <div>
            ...
            <CardDescription className="text-xs">{match.loadSnapshot.cargo}</CardDescription>
          </div>
      ...
      <CardContent className="space-y-1">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          {match.status === 'countered' && match.counterTerms && (
            <span className="text-muted-foreground">&rarr; ${match.counterTerms.rate.toLocaleString()}</span>
          )}
        </div>
      </CardContent>
    </Card>
...
export default function RequestsPage() {
  ...
  const { data: incomingMatches, isLoading: incomingLoading } = useCollection<Match>(incomingQuery);
  ...
  const { data: sentMatches, isLoading: sentLoading } = useCollection<Match>(sentQuery);

  const isExpired = (match: Match) => isPast(parseISO(match.expiresAt));
  const formatDate = (ds?: string) => { if (!ds) return "Not specified"; try { return format(parseISO(ds), "MMM d, yyyy"); } catch { return ds; } };
  ...
  const handleDeclineCounter = async () => {
    ...
    setIsDeclining(true);
    try {
      await updateDoc(doc(firestore, `matches/${selectedMatch.id}`), { status: 'declined', declinedAt: new Date().toISOString(), declinedBy: 'load_owner', declineReason: 'Counter offer declined' });
      const driverOwnerDoc = await getDoc(doc(firestore, `owner_operators/${selectedMatch.driverOwnerId}`));
      const info = driverOwnerDoc.exists() ? driverOwnerDoc.data() : null;
      if (info?.contactEmail) { notify.matchDeclined({ loadOwnerEmail: info.contactEmail, loadOwnerName: info.legalName || '', driverName: selectedMatch.driverSnapshot.name, loadOrigin: selectedMatch.loadSnapshot.origin, loadDestination: selectedMatch.loadSnapshot.destination, reason: 'Counter offer was not accepted' }).catch(err => console.error('Failed:', err)); }
      showSuccess("Counter offer declined."); setCounterModalOpen(false); setSelectedMatch(null);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold font-headline">Match Requests</h1>
        <p className="text-muted-foreground">Manage all your incoming and sent match requests in one place.</p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="incoming" className="flex items-center gap-2">
            ...
            {incomingPending.length > 0 && <Badge variant="default" className="ml-1 h-5 min-w-5 px-1.5 bg-red-600 hover:bg-red-600">{incomingPending.length > 99 ? '99+' : incomingPending.length}</Badge>}
          </TabsTrigger>
          <TabsTrigger value="sent" className="flex items-center gap-2">
            <Send className="h-4 w-4" />Sent
            {sentCountered.length > 0 && <Badge variant="default" className="ml-1 h-5 min-w-5 px-1.5 bg-blue-600 hover:bg-blue-600">{sentCountered.length > 99 ? '99+' : sentCountered.length}</Badge>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="incoming" className="space-y-6 mt-6">
          <div>
            ...
            {incomingPending.length === 0 ? (
              <Card><CardContent className="flex flex-col items-center justify-center py-12"><Inbox className="h-16 w-16 text-muted-foreground mb-4" /><h3 className="text-lg font-semibold">No Pending Requests</h3><p className="text-muted-foreground text-center mt-1">When someone sends a match request for your drivers or loads, it will appear here.</p></CardContent></Card>
            ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {incomingPending.map(m => <MatchCard key={m.id} match={m} type="incoming-pending" onRespond={handleRespond} getTimeLeft={getTimeLeft} formatDate={formatDate} isExpired={isExpired} />)}
              </div>
            )}
          </div>
          {incomingOther.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold mb-4">Past Requests</h2>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {incomingOther.map(m => <MatchCard key={m.id} match={m} type="incoming-past" getTimeLeft={getTimeLeft} formatDate={formatDate} isExpired={isExpired} />)}
              </div>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="sent" className="space-y-6 mt-6">
          {sentCountered.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold mb-4 flex items-center gap-2"><MessageSquare className="h-5 w-5 text-blue-600" />Counter Offers Received ({sentCountered.length})</h2>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {sentCountered.map(m => <MatchCard key={m.id} match={m} type="sent-counter" onViewCounter={handleViewCounter} getTimeLeft={getTimeLeft} formatDate={formatDate} isExpired={isExpired} />)}
              </div>
            </div>
          )}
          <div>
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2"><Clock className="h-5 w-5" />Awaiting Response ({sentPending.length})</h2>
            {sentPending.length === 0 ? (
              <Card><CardContent className="flex flex-col items-center justify-center py-12"><Send className="h-16 w-16 text-muted-foreground mb-4" /><h3 className="text-lg font-semibold">No Pending Requests</h3><p className="text-muted-foreground text-center mt-1">Your sent match requests will appear here.</p></CardContent></Card>
            ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {sentPending.map(m => <MatchCard key={m.id} match={m} type="sent-pending" getTimeLeft={getTimeLeft} formatDate={formatDate} isExpired={isExpired} />)}
              </div>
            )}
          </div>
          {sentOther.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold mb-4">Past Requests</h2>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {sentOther.map(m => <MatchCard key={m.id} match={m} type="sent-past" getTimeLeft={getTimeLeft} formatDate={formatDate} isExpired={isExpired} />)}
              </div>
            </div>
          )}
        </TabsContent>
      ...
      {selectedMatch && !isExpired(selectedMatch) && <MatchResponseModal open={responseModalOpen} onOpenChange={setResponseModalOpen} match={selectedMatch} onSuccess={() => setSelectedMatch(null)} />}

      <Dialog open={counterModalOpen} onOpenChange={setCounterModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><MessageSquare className="h-5 w-5 text-blue-600" />Counter Offer Received</DialogTitle>
            <DialogDescription>Review the counter offer and decide how to proceed.</DialogDescription>
          </DialogHeader>
          {selectedMatch && (
            <div className="space-y-4 py-4">
              <div className="p-4 bg-muted/50 rounded-lg">
                <h4 className="font-medium mb-2">Load</h4>
                <p className="font-semibold">{selectedMatch.loadSnapshot.origin} &rarr; {selectedMatch.loadSnapshot.destination}</p>
                <p className="text-sm text-muted-foreground">{selectedMatch.loadSnapshot.cargo} &bull; {selectedMatch.loadSnapshot.weight.toLocaleString()} lbs</p>
              </div>
              <div className="p-4 bg-muted/50 rounded-lg">
                <h4 className="font-medium mb-2">Driver</h4>
                <p className="font-semibold flex items-center gap-2"><User className="h-4 w-4" />{selectedMatch.driverSnapshot.name}</p>
                <p className="text-sm text-muted-foreground flex items-center gap-1"><MapPin className="h-3 w-3" />{selectedMatch.driverSnapshot.location}</p>
                <p className="text-sm text-muted-foreground flex items-center gap-1 mt-1"><Building2 className="h-3 w-3" />{selectedMatch.driverOwnerName}</p>
              </div>
              <div className="p-4 border-2 border-blue-500 rounded-lg bg-blue-50 dark:bg-blue-950/20">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-sm text-muted-foreground">Your Original Rate</p><p className="text-lg font-medium line-through">${selectedMatch.originalTerms.rate.toLocaleString()}</p></div>
                  <div><p className="text-sm text-blue-600 font-medium">Counter Rate</p><p className="text-2xl font-bold text-blue-600">${selectedMatch.counterTerms?.rate.toLocaleString()}</p></div>
                </div>
                {selectedMatch.counterTerms?.pickupDate && (<div className="mt-3 pt-3 border-t"><p className="text-sm text-muted-foreground">Counter Pickup Date</p><p className="font-medium">{formatDate(selectedMatch.counterTerms.pickupDate)}</p></div>)}
                {selectedMatch.counterTerms?.notes && (<div className="mt-3 pt-3 border-t"><p className="text-sm text-muted-foreground">Message from Driver Owner</p><p className="text-sm italic">&quot;{selectedMatch.counterTerms.notes}&quot;</p></div>)}
              </div>
              <p className="text-xs text-muted-foreground">Accepting will create a Trip Lease Agreement and enable messaging.</p>
            </div>
          )}
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={handleDeclineCounter} disabled={isAccepting || isDeclining} className="w-full sm:w-auto">
              {isDeclining ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <XCircle className="h-4 w-4 mr-2" />}Decline Counter
            </Button>
            <Button onClick={handleAcceptCounter} disabled={isAccepting || isDeclining} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700">
              {isAccepting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <CheckCircle className="h-4 w-4 mr-2" />}Accept Counter Offer
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
