...
const ATTESTATIONS = [
  {
    ...
    description:
      'We confirm that we maintain Driver Qualification Files (DQFs) for all drivers we invite or make available on the XtraFleet platform, in accordance with applicable federal and state regulations.',
  },
  {
    id: 'employmentCompliance',
    title: 'Employment & Compliance Responsibility',
    description:
      'We acknowledge that our company retains full responsibility for employment decisions, compliance determinations, and regulatory obligations related to our drivers.',
  },
  {
    id: 'verificationAuth',
    title: 'Verification Use Authorization',
    description:
      'We authorize XtraFleet to facilitate limited, transaction-based eligibility verification (e.g., license status, endorsements, Clearinghouse eligibility) on our behalf, subject to driver consent.',
  },
  {
    id: 'noRelianceDisclaimer',
    title: 'No Reliance Disclaimer',
    description:
      'We understand that verification results provided through XtraFleet are eligibility signals only and do not replace our independent compliance or safety obligations.',
  },
] as const;
...
function ComplianceContent() {
  ...
  const toggleCheck = (id: AttestationId) => {
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="mx-auto w-full max-w-2xl">
        <CardHeader className="space-y-4 text-center">
          ...
          <CardDescription>
            Please confirm the following to proceed. These acknowledgments are
            required to participate in the XtraFleet marketplace.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {ATTESTATIONS.map((attestation) => (
            ...
            >
              <Checkbox
                id={attestation.id}
                checked={checked[attestation.id]}
                onCheckedChange={() => toggleCheck(attestation.id)}
                className="mt-0.5"
              />
              <Label
