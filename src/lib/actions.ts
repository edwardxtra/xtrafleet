...
export async function createPaymentIntent(): Promise<string | { error: string }> {
  if (!process.env.STRIPE_SECRET_KEY || !stripe) {
    const errorMessage = 'Stripe has not been configured. Missing STRIPE_SECRET_KEY.';
...
export async function sendPasswordReset(
  ...
  try {
    ...
    const firebaseResetLink = await auth.generatePasswordResetLink(email, { url: 'https://xtrafleet.com/login' });
...
export interface AddDriverState { message: string; error: string; }

export async function inviteDriver(prevState: AddDriverState, formData: FormData): Promise<AddDriverState> {
  try {
    const user = await authenticateServerAction();
    if (!user) return { message: '', error: 'You must be logged in to invite a driver.' };

    const email = formData.get('email') as string;
    if (!email) return { message: '', error: 'Email is required.' };
    if (!resend) return { message: '', error: 'Email service not configured. Please contact support.' };
    ...
    try {
      ...
      await db.collection('driver_invitations').doc(invitationToken).set({
        email, ownerId: user.uid, createdAt: FieldValue.serverTimestamp(), expiresAt: Timestamp.fromDate(expiresAt), status: 'pending',
      });
    } catch (dbError) {
      console.error('Database error:', dbError);
      return { message: '', error: 'Failed to create invitation.' };
    }

    const invitationLink = `https://xtrafleet.com/driver-register?token=${invitationToken}`;
    
    const { data, error } = await resend.emails.send({
      from: 'XtraFleet <noreply@xtrafleet.com>', to: [email], subject: 'Invitation to Join XtraFleet',
      html: `<h1>You've been invited to join XtraFleet!</h1><p>${companyName ? `<strong>${companyName}</strong> has invited you` : 'You have been invited'} to join their fleet on XtraFleet.</p><p>Click the link below to create your driver profile:</p><a href="${invitationLink}" style="display: inline-block; padding: 12px 24px; background-color: #4F46E5; color: white; text-decoration: none; border-radius: 6px; margin: 16px 0;">Create Your Profile</a><p style="color: #666; font-size: 14px;">This invitation expires in 7 days.</p><p style="color: #666; font-size: 14px;">Or copy and paste this link: ${invitationLink}</p>`,
    });
